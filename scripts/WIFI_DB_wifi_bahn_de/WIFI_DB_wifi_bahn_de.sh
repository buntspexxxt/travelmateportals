...
curl -k -v $CURL_OPTS -c "$COOKIE_JAR" -o "$HTML_FILE" "https://${DOMAIN}/en/" 2>> "$LOG_FILE"
...
HTTP_STATUS=$(curl -k -v $CURL_OPTS -b "$COOKIE_JAR" -H "Cookie: csrf=$SEC_TOKEN" --data "login=true&CSRFToken=$SEC_TOKEN" -w "%{http_code}" -o /dev/null "https://${DOMAIN}/en/" 2>> "$LOG_FILE")
