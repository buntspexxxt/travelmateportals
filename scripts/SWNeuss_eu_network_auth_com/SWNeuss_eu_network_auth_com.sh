...
REDIRECT_URL=$(curl -k -L -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" -w "%{url_effective}" -o /dev/null "$CHECK_URL" | tr -d '\015')
...
HTML_CONTENT=$(curl -k -c "$COOKIE_FILE" -b "$COOKIE_FILE" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" "$REDIRECT_URL")
...
RESPONSE=$(curl -k -L -v -b "$COOKIE_FILE" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" "$GRANT_URL?continue_url=http://neverssl.com" 2>&1)
