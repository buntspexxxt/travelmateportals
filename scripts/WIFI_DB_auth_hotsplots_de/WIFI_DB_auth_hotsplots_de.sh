...
# Get initial page to extract hidden fields
curl -k -L -A "$USER_AGENT" -c "$COOKIE_FILE" -o "$HTML_FILE" -m 15 "http://neverssl.com" > /dev/null 2>&1
...
if [ -z "$CHALLENGE" ]; then
    ...
    # Form POST based on identified fields
    RESPONSE_CODE=$(curl -k -L -A "$USER_AGENT" -b "$COOKIE_FILE" -c "$COOKIE_FILE" -m 15 \
        ...
        -w "%{http_code}" -o /dev/null "https://auth.hotsplots.de/login")
