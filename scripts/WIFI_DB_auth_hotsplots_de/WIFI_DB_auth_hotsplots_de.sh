...
echo "Step 1: Initializing session and grabbing redirect URL..." | tee -a "$LOG_FILE"
...
echo "Step 2: Fetching portal page to extract dynamic tokens..." | tee -a "$LOG_FILE"
...
if [ -z "$TOKEN" ]; then
    echo "ERROR: Failed to extract tokens. Aborting." | tee -a "$LOG_FILE"
    exit 1
fi
