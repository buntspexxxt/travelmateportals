...
echo "Starting Conn4 portal login process..." | tee -a "$LOG_FILE"
...
EFFECTIVE_URL=$(curl -k -L -A "$USER_AGENT" -c "$COOKIE_FILE" -w "%{url_effective}" -o "$HTML_OUT" "https://rewe-wlan.conn4.com/" -m 15)

# 3. Extract Token and Submit
# Token is inside conn4.hotspot.wbsToken in the HTML
TOKEN=$(sed -n 's/.*"token":"\([^"]*\)".*/\1/p' "$HTML_OUT" | head -n 1)
