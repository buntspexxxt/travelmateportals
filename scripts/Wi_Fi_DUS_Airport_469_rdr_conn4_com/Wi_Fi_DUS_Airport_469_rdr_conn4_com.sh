...
curl -m 15 -k -v -A "$USER_AGENT" -c "$COOKIE_FILE" -o /tmp/index.html "https://469.rdr.conn4.com/"
...
curl -m 15 -k -v -A "$USER_AGENT" -b "$COOKIE_FILE" -c "$COOKIE_FILE" -X POST "https://469.rdr.conn4.com/wbs/de/roaming/return/" -d "token=$WBS_TOKEN"

echo "Step 4: Executing Scene Accept event..."
# The HTML/JS indicates the scene ID is 'agbRwik_7LwIN_lF'
curl -m 15 -k -v -A "$USER_AGENT" -b "$COOKIE_FILE" -c "$COOKIE_FILE" -X POST "https://469.rdr.conn4.com/scenes/agbRwik_7LwIN_lF/" -d "action=accept&terms=1"
