...
INITIAL_HTML=$(curl -v -A "$UA" -c "$COOKIE_FILE" "https://service.thecloud.eu/service-platform/macauthlogin/v5" 2>&1)

echo "Submitting registration form (Terms Acceptance)..." | tee -a "$LOG_FILE"
# The form has an action and a button, we POST the continue action.
# Extracting hidden fields if they exist (though this portal uses standard POST for this step)
