...
REDIRECT_RESP=$(curl -k -v -L -A "$UA" -c "$COOKIE_FILE" -b "$COOKIE_FILE" -w "%{url_effective}" -o /tmp/portal_page.html "http://neverssl.com/")
...
if [ -f /tmp/portal_page.html ]; then
    ...
    if [ -n "$FORM_ACTION" ]; then
        ...
        curl -k -v -A "$UA" -b "$COOKIE_FILE" -c "$COOKIE_FILE" -d "$POST_DATA" "$POST_URL"
    fi
fi
