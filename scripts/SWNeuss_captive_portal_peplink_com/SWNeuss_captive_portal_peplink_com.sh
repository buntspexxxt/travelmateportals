...
echo "Fetching initial portal parameters..." | tee -a "$LOG_FILE"
...
echo "Processing logic for automated connection..." | tee -a "$LOG_FILE"
# The portal requires a 'login' command. We build the full URL dynamically based on the observed JS redirect patterns.
LOGIN_URL="$BASE_URL/cp/login?$QUERY_STRING&command=login&resume=true&lang=en&_=$(date +%s)"

echo "Submitting final login request to: $LOGIN_URL" | tee -a "$LOG_FILE"
curl -k -m 15 -A "$USER_AGENT" -b "$COOKIE_FILE" -c "$COOKIE_FILE" "$LOGIN_URL" | tee -a "$LOG_FILE"
...
while [ $i -le 10 ]; do
    ...
    echo "Attempt $i: Not connected yet (HTTP Code: $CHECK_CODE). Waiting..." | tee -a "$LOG_FILE"
    sleep 4
    i=$((i + 1))
done
