...
RESPONSE=$(curl -k -L -A "$USER_AGENT" -c "$COOKIE_FILE" -w "%{url_effective}" -o "$HTML_OUT" "https://auth.hotsplots.de/login?res=notyet")
...
SUBMIT_RESPONSE=$(curl -k -L -A "$USER_AGENT" -b "$COOKIE_FILE" -c "$COOKIE_FILE" -d "$POST_DATA" -w "%{http_code}" -o /dev/null "https://auth.hotsplots.de/login")
