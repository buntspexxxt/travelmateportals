...
RESPONSE=$(curl -k -v -L -A "$USER_AGENT" -c "$COOKIE_FILE" -b "$COOKIE_FILE" "http://neverssl.com" 2>&1)
...
curl -k -v -A "$USER_AGENT" -c "$COOKIE_FILE" -b "$COOKIE_FILE" "$GET_ONLINE_URL" | tee -a "$LOG_FILE"
